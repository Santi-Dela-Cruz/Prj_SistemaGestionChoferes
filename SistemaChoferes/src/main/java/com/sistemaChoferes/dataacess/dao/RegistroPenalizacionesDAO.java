...
public class RegistroPenalizacionesDAO implements IDAO<RegistroPenalizaciones> {
    ...
    try {
        PreparedStatement checkPenalizaciones = connection
                .prepareStatement("SELECT n_Infracciones FROM registro_penalizaciones WHERE id_Chofer=? AND estado='A'");
    ...
    private void actualizarEstadoChofer(int idChofer) throws SQLException {
        PreparedStatement updateChofer = connection.prepareStatement("UPDATE choferes SET estado='X' WHERE id_Chofer=?");
